...
public class GeneratorCache {
    ...
    public static @Nullable CachedGenerationTaskResult getCache(Generator.GenOpts opts, byte[] imageHash, byte[] continuationHash) {
        String cacheKey = generateCacheKey(opts, imageHash, continuationHash);
        Fi dir = PicToTri.genCacheDir.child(cacheKey.substring(0, 2));
        ...
        try (Reads read = cacheFile.reads()) {
            ...
            return new CachedGenerationTaskResult(new Generator.GenerationOutput(shapes, generatedImage, cur, maxGen, acc));
    ...
    public static class CachedGenerationTaskResult extends ImageGenerationService.GenerationTaskResult {
        Generator.GenerationOutput generationOutput;

        public CachedGenerationTaskResult(Generator.GenerationOutput generationOutput) {
