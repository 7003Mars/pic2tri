...
//			Log.warn("Nothing marked?");
	...
	public void mark(ScanLine scanLine) {
		if (scanLine.y < 0 || scanLine.y >= this.height || scanLine.x1 == Integer.MIN_VALUE || scanLine.x2 < scanLine.x1) {
		...
		if ((testing || Prefs.debugMode) && marks.contains(l -> l.y == scanLine.y)) Log.warn("@ already there? For line @", scanLine.y, scanLine);
		marks.add(scanLine);
	}
