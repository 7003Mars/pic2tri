...
public class ImageConverter extends Converter {
    ...
    public ImageConverter(Layout<?> layout, Fi filePath) {
        ...
        for (Layout.ImageChunk<?> chunk : this.layout.chunks) {
            ...
            if (layout instanceof LogicDisplayLayout) {
                this.options.add(new Generator.GenOpts(0/*TODO*/, 175, totalShapes).retainPixmap(true));
            } else {
                // TODO VERY IMPT
                /*
                ...
                */
                this.options.add(new Generator.GenOpts(0/*TODO*/, 175, totalShapes).maxOut(1).retainPixmap(true));
            }
        }
    }
    ...
    public ConverterTask submit() {
        ...
        Pixmap processed = new Pixmap(resized.width, resized.height);
        CompletableFuture<Pixmap> imageFuture = new CompletableFuture<>();
        ConverterTask task = new ConverterTask(this, this.layout.chunks.size, imageFuture);
        AtomicInteger chunksDrawn = new AtomicInteger();
...
//            Log.info("Start @, @, w@ h@", ix, iy, iw, ih);
            ...
            ImageGenerationService.ImageGenerationTask genTask = ImageGenerationService.submitImage(options.get(i), cropped, null, 0, this);
            int finalI = i;
            genTask.output.thenAccept(out -> {
                task.completeChunk(finalI, out.shapes());
                synchronized (task) {
                    processed.draw(out.result(), 0, 0, iw, ih, ix, iy, iw, ih);
                    if (chunksDrawn.incrementAndGet() >= this.layout.chunks.size) {
                        imageFuture.complete(processed);
                    }
                }
            });
        }
